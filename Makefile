postgres:
	docker run --name --network bank-network postgres13 -p 5432:5432 -e POSTGRES_USER=root  -e POSTGRES_PASSWORD=aaa -d postgres:13
...
.PHONY: postgres createdb dropdb migrateup migrateup1 migratedown migratedown1 sqlc test server mock mock-mac